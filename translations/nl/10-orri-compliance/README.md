# ORRI-naleving

Hoe Sulitânia de 10 principes van Open en Verantwoordelijk Onderzoek en Innovatie toepast.

## Wat hier staat

- **principles-mapping.md** — Elk ORRI-principe gekoppeld aan bewijsmateriaal in de repository
- **self-assessment.md** — Periodieke eerlijke zelfevaluatie per principe
- **future-funding.md** — DLBC, LEADER, volgende REINFORCING-ronde

## De 10 ORRI-principes

| # | Principe | Bewijs in deze repo |
|---|---|---|
| 1 | Inclusief bestuur | `01-governance/` |
| 2 | Maatschappelijke doelen | `02-ecology/`, `03-social/`, `04-economy/` |
| 3 | Multi-stakeholder (Quadruple Helix) | `03-social/stakeholders/` |
| 4 | Anticipatie en reflexiviteit | `09-learning/quarterly-reflections/` |
| 5 | Marktfalen aanpakken | `04-economy/circulation-model.md` |
| 6 | Representatie verbreden | `03-social/onboarding/` |
| 7 | Monitoring en verantwoording | `09-learning/indicators.md` |
| 8 | Experimentele aanpak | `02-ecology/ssaaflab/`, `06-housing/` |
| 9 | Ethiek en integriteit | `09-learning/mistakes-and-lessons.md` |
| 10 | Wetenschapseducatie en burgerschap | `03-social/education/` |

## Kerndocumenten

De grondslagdocumenten van Sulitânia staan in `docs/` (EN) en `translations/nl/docs/` (NL):
- **Heal the System** — systeemanalyse en ORRI-context
- **The Living Trinity** — filosofie en waarden
- **SYFERS Voorstel** — de ORRI Incubator-aanvraag
- **Communicatieplan** — outreachstrategie

---
*Laatste update: april 2026*
